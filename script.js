...
window.onload = () => {
  ...
  showFeaturedMovie = (movieId, movieName, movieDescription, movieImage) => {
    ...
    const featuredMovieDescription = document.querySelector('.featured__description')
    ...
    for (let movie of movies.results) {
        ...
        if (dom_element == '#trending') {
          moviesImages.onclick = () => {
            showFeaturedMovie(movie.id, movie.original_title, movie.overview, backdropImageUrl)
          }
        } else {
          moviesImages.onclick = () => showFeaturedMovie(movie.id, movie.name, movie.overview, backdropImageUrl)
        }
